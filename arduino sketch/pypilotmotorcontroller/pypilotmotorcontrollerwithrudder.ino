 ...
 */
  ...
  */
  ...
  /*
   * Modified by Andreas Walther
   * change the pwm output pins 
   * set the pwm frequency to 31 kHz
   * no status display (don´t need this cause all needed informations are shown on the lcd from pypilot or in the OpnCPN plugin)
   * change values from the ntc
   */
...
TCCR1B = TCCR1B & 0b11111000 | 0x01; //Setze Timer1 auf 31kHz PWM Frequenz um Pfeifen vom Motor zu verhindern   
